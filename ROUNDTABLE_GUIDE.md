# The Roundtable -- Silent Honor Decision-Review Guide

**TL;DR:** The Roundtable is a decision-review process for calls that are expensive or
slow to undo for Silent Honor -- program changes, partnerships, hiring, platform
...
This process was adapted from a shared, org-wide "panel of 5 advisors -- cross-review --
...
- A **fact-check stage runs first, every time** -- every checkable claim in the decision
  gets verified (confirmed / corrected / flagged unverifiable) before any seat argues.
- The verdict states disagreement as a **vote** (e.g. "3-of-5 favor A") -- scannable in
  seconds, not buried in hedged prose.
- Same 5 personas by default, **plus** optional 6th/7th seats (Compliance, Security,
  Brand, Financial) added -- never substituted -- when the decision calls for it. For
...
Nothing to install -- the skill lives in this repo at
...
Claude runs the full 4-stage process (fact-check -- independent -- cross-review -- verdict),
...
Don't use it for small reversible stuff -- "should this button be blue or navy" doesn't
...
Before any seat answers, Claude pulls every checkable claim out of the decision --
numbers, dates, comparisons, "we have X members enrolled," "this costs $Y" -- and
...
had right in the first place -- only checking the number itself does.

## The five seats

| Seat | Job |
|---|---|
| **The Contrarian** | Finds the single fatal flaw -- not a balanced pros/cons list |
...
| **The Outsider** | Zero-context reaction -- catches curse-of-knowledge blind spots |
| **The Executor** | Only cares what you do Monday morning -- one action, one owner, one date |
...
## The ledger -- every verdict gets logged

Every verdict is appended to `ROUNDTABLE_LOG.md` at the repo root -- one entry per
...
- **Decision:** <one line>
- **Facts checked:** <what was verified/corrected, or "none -- no checkable claims in this one">
...
## Reading a verdict -- one red flag to know

If a verdict comes back **unanimous with zero real tension**, that's not proof the
decision is safe -- it's a signal to check that the independent stage actually ran blind.
...
surfaces something that's really someone's call to make, ask them directly -- the
Executor seat doesn't decide it on their behalf.

**Does a Roundtable Verdict grant merge/deploy/spend authority?** No. It's a
recommendation only.

**Can I add my own seat permanently?** No -- the 5 fixed seats stay fixed so verdicts stay
...
cross-review stage -- that's exactly the loop this is built to catch.

**Where do past verdicts live?** `ROUNDTABLE_LOG.md` at the repo root -- every verdict
